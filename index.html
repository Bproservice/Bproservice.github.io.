...
<head>
    ...
    <style>
        ...
        * {
        ...
        .logo-text { display: flex; flex-direction: column; font-size: 0.95rem; }
        ...
        .hero-list span { display: flex; align-items: center; gap: 6px; }
        .hero-list span::before { content: "✓"; color: var(--accent); font-size: 0.95rem; }
        ...
        .hero-illu-text { font-size: 0.9rem; }
        ...
        .hero-stats strong { display: block; font-size: 1.1rem; color: var(--primary-dark); }
        ...
        .section-subtitle { font-size: 0.95rem; color: var(--text-muted); max-width: 540px; }
        ...
        .card:hover { transform: translateY(-3px); box-shadow: 0 10px 26px rgba(15, 23, 42, 0.12); }
        ...
        .chip-row { margin-top: 8px; display: flex; flex-wrap: wrap; gap: 4px; }
        .chip { font-size: 0.75rem; padding: 3px 8px; border-radius: 999px; background: #f1f3f5; color: var(--text-muted); }

        /* POUR QUI */
        .grid-2 { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 22px; }

        .list { list-style: none; font-size: 0.9rem; color: var(--text-muted); }
        .list li { position: relative; padding-left: 18px; margin-bottom: 6px; }
        .list li::before { content: "•"; position: absolute; left: 5px; top: 0; color: var(--primary); }

        /* PROCESS */
        .steps { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 14px; margin-top: 16px; }
        .step { background: #f5f7fb; border-radius: 14px; padding: 12px; font-size: 0.85rem; }
        .step-number {
            width: 24px; height: 24px; border-radius: 999px; background: #ffffff;
            display: flex; align-items: center; justify-content: center;
            font-size: 0.85rem; font-weight: 600; color: var(--primary);
            margin-bottom: 4px;
        }
        .step h3 { font-size: 0.9rem; margin-bottom: 3px; }

        .pill-row { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 12px; }
        .pill { font-size: 0.8rem; padding: 6px 10px; border-radius: 999px; border: 1px solid var(--border); background: #fff; color: var(--text-muted); }

        /* TEMOIGNAGES */
        .testimonials { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 16px; margin-top: 12px; }
        .testimonial { background: #ffffff; border-radius: 14px; padding: 14px; border: 1px solid var(--border); font-size: 0.88rem; color: var(--text-muted); }
        .testimonial strong { display: block; margin-top: 8px; color: var(--text-main); }

        /* FAQ (INTERACTIF) */
        .faq { margin-top: 18px; }
        .faq-item { border-radius: 12px; border: 1px solid var(--border); padding: 10px 12px; margin-bottom: 8px; background: #ffffff; cursor: pointer; }
        .faq-question { display: flex; justify-content: space-between; align-items: center; font-size: 0.92rem; font-weight: 500; }
        .faq-toggle { font-size: 1.2rem; color: var(--primary); }
        .faq-answer { margin-top: 6px; font-size: 0.86rem; color: var(--text-muted); display: none; }
        ...
        .contact-block { font-size: 0.92rem; color: var(--text-main); }
        ...
        /* Mobile encore plus confortable */
        ...
        <div class="container">
            ...
            <div class="contact-grid">
                ...
                <div>
                    ...
                    <form id="contact-form" action="https://formspree.io/f/xykkkbjv" method="POST">
                        <input type="text" name="_gotcha" style="display:none">
                        <input type="hidden" name="_subject" value="Nouvelle demande de devis – B-Pro Service">

                        <!-- ✅ URL CORRIGÉE (c'était la source du bug) -->
                        <input type="hidden" name="_next" value="https://bproservice.github.io/#merci">
                        ...
                        <p style="font-size:0.8rem; color:#666; margin-top:6px;">
                            Après l’envoi, vous recevrez une confirmation sur cette page.
                        </p>
                    </form>
                </div>
            </div>
...
<script>
    ...
    // Affichage du message "Merci" après redirection
    const hash = window.location.hash;
    const params = new URLSearchParams(window.location.search);

    if (hash === "#merci" || params.get("success") === "1") {
        const merci = document.getElementById("merci");
        if (merci) {
            merci.style.display = "block";
            merci.scrollIntoView({ behavior: "smooth" });
        }
    }
</script>
